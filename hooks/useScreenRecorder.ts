  ...
  const startRecording = useCallback(
    async (resolution: '1080p' | '4K' = '4K') => {
      try {
        ...
        console.log(
          '[ScreenRecorder] Screen picker resolved, tracks:',
          displayStream.getTracks().length,
        );
        ...
        if (hasAudio && audioTracks.length > 0) {
          const meterSource = audioCtx.createMediaStreamSource(new MediaStream([audioTracks[0]]));
        ...
        const checkBothStopped = () => {
          if (recorderStopped && audioRecorderStopped) {
            console.log(
              '[ScreenRecorder] Both recorders stopped, creating final blob. Chunks:',
